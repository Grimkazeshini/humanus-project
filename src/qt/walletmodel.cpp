...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered humanus address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HMS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal humanus:URI (humanus:XyZ...?message=example)
            {
...
static void NotifyzHMSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzHMSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzHMSReset.connect(boost::bind(NotifyzHMSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzHMSReset.disconnect(boost::bind(NotifyzHMSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zhmsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
