...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeHumanusAmount"))
        settings.setValue("nAnonymizeHumanusAmount", 1000);

    nAnonymizeHumanusAmount = settings.value("nAnonymizeHumanusAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHumanusAmount"))
        SoftSetArg("-anonymizehumanusamount", settings.value("nAnonymizeHumanusAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in humanus.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHumanusAmount:
            return QVariant(nAnonymizeHumanusAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHumanusAmount:
            nAnonymizeHumanusAmount = value.toInt();
            settings.setValue("nAnonymizeHumanusAmount", nAnonymizeHumanusAmount);
            emit anonymizeHumanusAmountChanged(nAnonymizeHumanusAmount);
            break;
        case CoinControlFeatures:
