...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Humanus address (e.g. %1)").arg("R7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no Humanus: URI
...
{
    // Convert humanus:// to humanus:
    //
    //    Cannot handle this later, because humanus:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open humanus.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Humanus.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Humanus.lnk
...
{
    return GetAutostartDir() / "humanus.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a humanus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Humanus\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the humanus app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add humanus app to startup item list
