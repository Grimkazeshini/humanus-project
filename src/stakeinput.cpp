...
// The zHMS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zHMS is a hash of the serial
...
{
    //Create an output returning the zHMS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZHMSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zHMS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zHMS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZHMSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zHMS output", __func__);
...
{
    CzHMSTracker* zhmsTracker = pwallet->zhmsTracker.get();
    CMintMeta meta;
    if (!zhmsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zhmsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
