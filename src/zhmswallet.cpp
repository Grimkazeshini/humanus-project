...
CzHMSWallet::CzHMSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhms seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHMSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHMSSeed_deprecated()) {
                    LogPrintf("%s: Updated zHMS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhms seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhms wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHMSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHMSCount(nCountLastUsed);
    else if (!walletdb.ReadZHMSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHMSWallet::Lock()
{
    seedMaster = 0;
}

void CzHMSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHMSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHMS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHMSWallet::LoadMintPoolFromDB()
{
...
void CzHMSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHMSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHMSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhmsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHMSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHMS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhmsTracker which also adds to database
    pwalletMain->zhmsTracker->Add(dMint, true);
...
void CzHMSWallet::SeedToZHMS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHMSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHMSWallet::GenerateDeterministicZHMS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHMSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHMSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
