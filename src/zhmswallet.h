...
#ifndef Humanus_ZHMSWALLET_H
#define Humanus_ZHMSWALLET_H
...
public:
    CzHMSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZHMS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZHMS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Humanus_ZHMSWALLET_H
