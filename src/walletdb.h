...
public:
    ...
    bool WriteZHMSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHMSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHMSSeed_deprecated(uint256& seed);
    bool EraseZHMSSeed();
    bool EraseZHMSSeed_deprecated();

    bool WriteZHMSCount(const uint32_t& nCount);
    bool ReadZHMSCount(uint32_t& nCount);
